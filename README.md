...
- `nt2json`, `nt2toml`, `nt2yaml`, `nt2huml`
- `json2nt`, `toml2nt`, `yaml2nt`, `huml2nt`
...
Support for TOML and HUML are optional.
If you don't need support for those, you can omit the `[toml,huml]` bits below.

Here are some ways to install it:

```console
$ uv tool install 'nt2[toml,huml]'     # Install using uv (Python all-around manager)
$ pipx install 'nt2[toml,huml]'        # Install using pipx (Python app manager)
$ pipz install 'nt2[toml,huml]'        # Install using zpy (Python app and environment manager for Zsh)
$ pip install --user 'nt2[toml,huml]'  # Install in your user's environment
$ pip install 'nt2[toml,huml]'         # Install in current environment
```
...
```zsh
if (( $+functions[compdef] )) && (( $+commands[nt2json] ))  compdef _gnu_generic nt2{json,{to,ya,hu}ml} {json,{to,ya,hu}ml}2nt
```
